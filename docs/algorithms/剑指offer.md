...
2方法2：这种类别的题目，如果直观求解不行的话，那么通常是进行找规律，转化成一个数学问题。这道题目在《编程之美》上有着比较详细的描述，下面就结合一个实例进行具体的分析：
...
# -*- coding:utf-8 -*-
    public class Solution {
       1. public int NumberOf1Between1AndN_Solution(int n) {
                ...
                int i =1;        //从个位开始
                int x =0;
                while (high != 0) {     //高位为0,则退出
                        high = n /(int)Math.pow(10,i); //高位
                        int tmp =n%(int)Math.pow(10,i);
                        x = tmp/(int)Math.pow(10, i-1); //第i位上的数字
       ...
       2.     public int NumberOfKBetween1AndN_Solution(int k,int n) {
                    if (n < 1 || k<0 || k>9){   //数字k不能为0到9
                        return 0;
                    }
                    int count = 0;
                    int high = n;     //高位
